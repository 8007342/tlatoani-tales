...
| ~~ME10~~ | *(tombstoned 2026-04-22)* | — | — | Reserved as a candidate ("exit 0 is not success"); author declined — not a load-bearing teaching beat. ID never reuses. Tombstone itself is an on-brand demonstration of the ledger's CRDT mechanic. |
...
| ME13 | **Lesson-trace coverage as humane CRDT** | C05, C08, C09, C12 | `lessons/spec.md` | Each lesson slug accretes coverage — specs and meta-examples it touches — as a set-union CRDT. Strips point to one primary lesson; the lesson spec points back to everything. Observability on top of observability: `@Lesson <slug>` is the telemetry of the trace layer itself. Reader's door into the whole wisdom graph is one click. |
